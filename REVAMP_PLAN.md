...
- **5b (⚠ needs install to verify):** new `packages/native-host` — Node host: length-prefixed-JSON stdio protocol, `better-sqlite3` writer that **owns the schema** (memories incl. `kind` column + FTS5 + conversations) and upserts idempotently by id, per-OS manifest installer CLI (`engram-native-host-install <ext-id>`). No deps installed in this workspace → not built/run here.
