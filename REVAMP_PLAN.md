...
### Phase 5 — The live bridge (native messaging) — 5a DONE; 5b/5c needs-install
- **5a (✅ validated):** extension-side bridge. `lib/bridge.ts` (pure, tested) — outbox of memory IDs in `chrome.storage.local` (never plaintext at rest), payload builder, injectable drain (drops gone entries, re-queues on channel error). `lib/bridge-runtime.ts` — `connectNative` glue + in-SW decrypt (throws when locked so delivery defers, not lost). Hooked into save (best-effort) + startup drain. `nativeMessaging` permission. 7 new tests; 666 green.
- **5b (✅ unit-verified):** new `packages/native-host` — Node host: length-prefixed-JSON stdio protocol, `better-sqlite3` writer that **owns the schema** (memories incl. `kind` column + FTS5 + conversations) and upserts idempotently by id, per-OS manifest installer CLI (`engram-native-host-install <ext-id>`). Installed + built + smoke-tested (`npm test`): schema, idempotent upsert, FTS5 search, read-only read-back, conversation roll-up, protocol framing all pass. **Running the test caught two real bugs:** `INSERT OR REPLACE` corrupted the external-content FTS5 index (→ `ON CONFLICT DO UPDATE`), and re-sends inflated `message_count` (→ recompute from rows). Live browser↔host handshake still needs a manual round-trip.
- **5c (⚠ needs install to verify):** MCP server made **read-only** — `SQLiteStorage` opens `{ readonly, fileMustExist }` and skips schema creation; `index.ts` opens read-only; `server.ts` no longer registers import/bulk-write tools. Closes R1 (single-writer by construction). MCP package isn't installed here → unverified.
- **Validation needed on install:** `cd packages/native-host && npm install && npm run build`; install host manifest; build MCP; exercise a real save → host write → MCP read round-trip.

#### Original Phase 5 design notes
### Phase 5 (orig) — The live bridge (native messaging)
